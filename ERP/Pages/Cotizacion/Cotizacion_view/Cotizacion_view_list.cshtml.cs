                ...
                using (SqlDataReader reader = command.ExecuteReader()) // Ejecutamos el lector
                {
                    while (reader.Read())
                    {
                        CotizacionInfo cotizacion = new CotizacionInfo
                        {
                            ...
                            monto_total = reader.GetDouble(3).ToString(),
                            mes_cierre = reader.GetInt32(4).ToString(),
                            probabilidad = reader.GetDouble(5).ToString(),
                            ...
                            zona = reader.GetString(11),       // Asume que es un ID entero
                            sector = reader.GetString(12).ToString(),     // Asume que es un ID entero
                            estado = reader.GetString(13).ToString(),     // Asume que es un ID entero
                            tipo = reader.GetString(14).ToString()
                        };
